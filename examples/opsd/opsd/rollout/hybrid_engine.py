...
The continuous batching path is activated when
``RolloutConfig.continuous_batching_size > 0``.
"""
...
class HybridEngineRollout(RolloutEngine):
    ...
    def generate(self, request: RolloutRequest, sampling: SamplingConfig) -> RolloutBatch:
        cb_size = getattr(self.cfg, "continuous_batching_size", 0)
        if cb_size > 0:
    ...
    def _generate_continuous_batching(self, request: RolloutRequest, sampling: SamplingConfig,
        ...
        B = request.prompt_ids.shape[0]  # number of prompts in micro-batch
        ...
        try:
            ...
            if B == 1:
                ...
                # Multiple prompts: interleave n samples per prompt
            ...
            next_tokens = self._sample_top_p(init_logits, temperature, top_p)  # [init_count, 1]
            ...
            while completed_count < total:
                ...
                for i in range(num_slots):
                    ...
                    if eos_mask[i] or slot_decode_step[i] >= max_new_tokens:
                        completed_count += 1
                        if next_rollout_idx < total:
                            # CB: replace slot with next pending rollout
                            self._cb_replace_slot(
                                i, past, prompt_past, attn_mask, first_logits,
                                next_tokens, all_tokens, gen_lens, slot_rollout,
                                slot_decode_step, slot_position, slot_pad_start,
                            ...
                            # Early exit: mark slot for removal
        ...
        # Attention mask: prompt mask + response mask (non-pad tokens)
    ...
    def _cb_replace_slot(self, slot_idx, past, prompt_past, attn_mask,
        ...
        # Determine which prompt this rollout belongs to
        ...
        # Replace attention mask: [0...0(pad_len), 1...1(prompt_len)]
        ...
        # Sample first token for new rollout from cached prefill logits
    ...
    def _sample_top_p(self, logits, temperature=1.0, top_p=1.0):
