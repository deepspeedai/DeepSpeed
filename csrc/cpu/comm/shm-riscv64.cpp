...
inline vfloat32m4_t cvt_bf16_to_fp32(vuint16m2_t src, size_t vl)
{
    vuint32m4_t widened = __riscv_vwcvtu_x_x_v_u32m4(src, vl);
    vuint32m4_t shifted = __riscv_vsll_vx_u32m4(widened, 16, vl);
    vfloat32m4_t fp32_val = __riscv_vreinterpret_v_u32m4_f32m4(shifted);
    return fp32_val;
}

inline vuint16m2_t cvt_fp32_to_bf16(vfloat32m4_t src, size_t vl)
{
    vuint32m4_t value = __riscv_vreinterpret_v_f32m4_u32m4(src);
    vuint32m4_t nan = __riscv_vmv_v_x_u32m4(0xFFFF, vl);
    vbool8_t mask_value = __riscv_vmfne_vv_f32m4_b8(src, src, vl);
    uint8_t mask_storage[8];
    __riscv_vsm_v_b8(mask_storage, mask_value, vl);
    vuint32m4_t ones = __riscv_vmv_v_x_u32m4(0x1, vl);
    vuint32m4_t vec_bias = __riscv_vmv_v_x_u32m4(0x7FFF, vl);
    vuint32m4_t t_value = __riscv_vand_vx_u32m4(__riscv_vsrl_vx_u32m4(value, 16, vl), 0x1, vl);
    t_value = __riscv_vadd_vv_u32m4(t_value, vec_bias, vl);
    value = __riscv_vadd_vv_u32m4(t_value, value, vl);
    value = __riscv_vsrl_vx_u32m4(value, 16, vl);
    vuint32m4_t result = __riscv_vmerge_vvm_u32m4(value, nan, mask_value, vl);

    return __riscv_vncvt_x_x_w_u16m2(result, vl);
}

inline vfloat32m4_t cvt_fp16_to_fp32(vfloat16m2_t src, size_t vl)
{
    return __riscv_vfwcvt_f_f_v_f32m4(src, vl);
}

inline vfloat16m2_t cvt_fp32_to_fp16(vfloat32m4_t src, size_t vl)
{
...
#define CVT_ADD_BF16(x)                                                                   \
    do {                                                                                  \
        auto in##x##_val =                                                                \
            cvt_bf16_to_fp32(__riscv_vle16_v_u16m2((uint16_t*)(buffers[x] + i), vl), vl); \
        inout_val = __riscv_vfadd_vv_f32m4(inout_val, in##x##_val, vl);                   \
    } while (0)
...
#pragma omp parallel for
    for (int i = start_elements * element_size; i < (start_elements + main_elements) * element_size;
        ...
        auto inout_val =
            cvt_bf16_to_fp32(__riscv_vle16_v_u16m2((uint16_t*)(buffers[0] + i), vl), vl);
        switch (world_size) {
            ...
            default:
                for (int j = 1; j < world_size; j++) {
                    auto in_val = cvt_bf16_to_fp32(
                        __riscv_vle16_v_u16m2((uint16_t*)(buffers[j] + i), vl), vl);
                    inout_val = __riscv_vfadd_vv_f32m4(inout_val, in_val, vl);
                }
        }
        __riscv_vse16_v_u16m2((uint16_t*)(to_buffer + i), cvt_fp32_to_bf16(inout_val, vl), vl);
    }
...
#define CVT_ADD_FP16(x)                                                                    \
    do {                                                                                   \
        auto in##x##_val =                                                                 \
            cvt_fp16_to_fp32(__riscv_vle16_v_f16m2((float16_t*)(buffers[x] + i), vl), vl); \
        inout_val = __riscv_vfadd_vv_f32m4(inout_val, in##x##_val, vl);                    \
    } while (0)
...
#pragma omp parallel for
    for (int i = start_elements * element_size; i < (start_elements + main_elements) * element_size;
         i += VECTOR_LENGTH_IN_BYTES) {
        auto inout_val =
            cvt_fp16_to_fp32(__riscv_vle16_v_f16m2((float16_t*)(buffers[0] + i), vl), vl);
        switch (world_size) {
            ...
            default:
                for (int j = 1; j < world_size; j++) {
                    auto in_val = cvt_fp16_to_fp32(
                        __riscv_vle16_v_f16m2((float16_t*)(buffers[j] + i), vl), vl);
                    inout_val = __riscv_vfadd_vv_f32m4(inout_val, in_val, vl);
                }
        }
        __riscv_vse16_v_f16m2((float16_t*)(to_buffer + i), cvt_fp32_to_fp16(inout_val, vl), vl);
    }
...
#define CVT_ADD_F32(x)                                                          \
    do {                                                                        \
        auto in##x##_val = __riscv_vle32_v_f32m2((float*)(buffers[x] + i), vl); \
...
#pragma omp parallel for
    for (int i = start_elements * element_size; i < (start_elements + main_elements) * element_size;
         i += VECTOR_LENGTH_IN_BYTES) {
        auto inout_val = __riscv_vle32_v_f32m2((float*)(buffers[0] + i), vl);
        switch (world_size) {
            ...
            default:
                for (int j = 1; j < world_size; j++) {
                    auto in_val = __riscv_vle32_v_f32m2((float*)(buffers[j] + i), vl);
                    inout_val = __riscv_vfadd_vv_f32m2(inout_val, in_val, vl);
                }
        }
        __riscv_vse32_v_f32m2((float*)(to_buffer + i), inout_val, vl);
    }
...
static void parallel_memcpy(void* to, void* from, size_t n_bytes)
{
    ...
    size_t vl = VECTOR_LENGTH_IN_BYTES / 2;
    // process aligned part
#pragma omp parallel for
    for (int i = 0; i < aligned_bytes; i += VECTOR_LENGTH_IN_BYTES) {
        auto val = __riscv_vle16_v_u16m2((uint16_t*)((char*)from + i), vl);
        __riscv_vse16_v_u16m2((uint16_t*)((char*)to + i), val, vl);
    }
