...
# DeepSpeed Team
"""
Test that ZeRO Stage 2 uses the GPU flatten path when VRAM is sufficient.
Runs for fp32, fp16, or bf16 depending on accelerator support.
"""
...
class TestStage2FlattenOnGPU(DistributedTest):
    """ZeRO-2 with small model should flatten on GPU (sufficient VRAM)."""

    world_size = 2  # Run on 2 GPUs when available

    def test_flatten_on_gpu_path_taken(self, monkeypatch):
    ...
    def test_flat_buffers_on_accelerator(self):
        ...
        assert hasattr(opt, "bit16_groups_flat"), "ZeRO-2 optimizer should have bit16_groups_flat"
