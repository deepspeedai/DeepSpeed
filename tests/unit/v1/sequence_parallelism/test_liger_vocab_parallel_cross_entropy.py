...
def test_liger_cpu_fallback_matches_torch():
...
class TestLigerVocabParallelCE(DistributedTest):
    ...
    def test_losses_and_gradients(self):
        ...
        if not str(device).startswith("cuda"):
            pytest.skip("Liger CE requires CUDA")
        rank = dist.get_rank()
    ...
    def test_five_optimizer_steps(self, vocabulary):
        ...
        try:
            ...
            for step in range(5):
                ...
                if rank == 0:
                    print(f"PR-D vocab={vocabulary} step={step + 1} loss={actual.item():.6f} reference-aligned",
                          flush=True)
        finally:
            reset_tp_model_init_state()
