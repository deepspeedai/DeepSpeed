...
class TestUnfusedOptimizerGradNorm(DistributedTest):
    ...
    @pytest.mark.parametrize("fused_lamb_legacy", [False, True])
    def test_reported_grad_norm_survives_a_scale_increase(self, fused_lamb_legacy):
        # Dynamic scaling raises cur_scale inside step(), before the norm is reported, so the
        # scale in hand at that point is the NEXT one while the gradients still carry the
        # previous one. Dividing by cur_scale there halves the reported norm on exactly the
        # steps where the scale moves; a static scale never moves and cannot see it.
        true_grad_value = 0.25
        expected_norm = 1.0
        params = [torch.nn.Parameter(torch.zeros(8, dtype=torch.float16)) for _ in range(2)]
        ...
        reported, scales = [], []
        for _ in range(6):
            # What backward() would have produced: the true gradient times the scale in force.
            scale_at_backward = optimizer.loss_scale_config.cur_scale
            for p in params:
                p.grad = torch.full_like(p, true_grad_value * scale_at_backward)
            optimizer.step()
            scales.append((scale_at_backward, optimizer.loss_scale_config.cur_scale))
            reported.append(optimizer._global_grad_norm)

        assert any(before != after
                   for before, after in scales), (f"the scale never moved, so this test proves nothing: {scales}")
