...
def split_half_float_double_sparse(tensors):
    ...
    for t in tensors:
        assert t.dtype in supported_types, (
...
class DeepSpeedEngine(Module):
    ...
    def _do_args_sanity_check(self, args):
        ...
        if hasattr(args, "local_rank") and args.local_rank is not None:
            assert isinstance(args.local_rank, int), (
    ...
    def _do_optimizer_sanity_check(self, basic_optimizer):
        ...
        assert not (amp_enabled and zero_enabled), (
    ...
    def _configure_basic_optimizer(self, model_parameters):
        ...
        if self.optimizer_name() in [ADAM_OPTIMIZER, ADAMW_OPTIMIZER]:
            ...
            zero_stage = self.zero_optimization_stage()
            assert zero_stage <= ZeroStageEnum.gradients, (
                "Muon optimizer is not yet compatible with ZeRO Stage 3"
            )
        ...
        if zero_stage <= ZeroStageEnum.gradients:
            ...
            if isinstance(optimizer, DummyOptim):
                ...
                if self.zero_allgather_sequential():
                    log_dist(
                        f"If zero_allgather_sequential is True, set prefetch_bucket_size to 1",
                        ranks=[0],
                    )
    ...
    def _backward_prologue(self):
        ...
        assert not self.eigenvalue_enabled(), (
    ...
    def _load_zero_checkpoint(self, load_dir, tag, load_optimizer_states=True):
        ...
        if self._config.zero_config.pipeline_loading_checkpoint:
            assert self.zero_optimization_stage() == ZeroStageEnum.weights, (
    ...
    def _save_moe_checkpoint(
        ...
        if groups._get_data_parallel_rank() == 0:
            ...
            state = {
                ...
                "data_sampler": self.training_dataloader.data_sampler.state_dict()
                if (
                    self.training_dataloader is not None
                    and self.curriculum_learning_enabled()
                )
    ...
    def _save_checkpoint(
        ...
        state = dict(
            ...
            frozen_param_shapes=self._get_zero_frozen_param_attributes(
                self._get_param_shape_func
            )
            ...
            frozen_param_fragments=self._get_zero_frozen_param_attributes(
                self._get_param_fragment_func
            )
            if save_frozen_param
            else None,
            lr_scheduler=self.lr_scheduler.state_dict()
    ...
    def compile(
        ...
        if enable_deepcompile:
            ...
            assert backend in ["inductor", "eager"], (
    ...
    def offload_states(
        ...
        assert not isinstance(self.optimizer, DeepSpeedZeRoOffload), (
