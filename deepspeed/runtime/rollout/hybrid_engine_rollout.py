        ...
        try:
            ...
            if self.use_graph_capture and is_greedy:
                ...
                output_ids = module.generate(
                    ...
                    # Transformers enables synced_gpus from the default world
                    # size when it can observe ZeRO-3, and the decode loop then
                    # issues an ungrouped world all-reduce. A data-parallel
                    # subgroup that enters rollout alone would block on that
                    # collective. The criterion above already synchronizes the
                    # stop decision over the parameter-gather group, so opt out
                    # of the native synchronization for the ZeRO-3 path only.
                    synced_gpus=False if zero3_sync is not None else None,
                    do_sample=do_sample,
