...
def _initialize_zenflow_stage3_epilogue(optimizer_z3: "DeepSpeedZeroOptimizer_Stage3",
    ...
    if optimizer_z3.zenflow_overlap_step:
        optimizer_z3.process_pool = mp.Pool(1)
...
def start_optimizer_process(optimizer_z3):
    ...
    optimizer_z3.parent_conn, optimizer_z3.child_conn = Pipe()

    manager = Manager()
    optimizer_z3.shared_overlap_grad_map = manager.dict()
    optimizer_z3.shared_stale_param_map = manager.dict()
    for group_no, param in enumerate(optimizer_z3.fp32_partitioned_groups_flat):
        ...
        if not hasattr(param, 'stale_param'):
            ...
            key = group_no
            optimizer_z3.shared_stale_param_map[key] = param.stale_param
        if param.overlap_grad is not None:
            param.overlap_grad[0].data.share_memory_()
            param.overlap_grad[1].data.share_memory_()
            key = group_no
            optimizer_z3.shared_overlap_grad_map[key] = param.overlap_grad

    param_groups_data = [{"params": group} for group in optimizer_z3.fp32_partitioned_groups_flat]
    ...
    optimizer_z3.process_optimizer_established = True
