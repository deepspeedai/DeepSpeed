...
def get_global_norm_of_tensors(input_tensors, norm_type=2, mpu=None, moe_ep_group=None):
    ...
    if norm_type == inf:
        ...
        if 'norm_tensors_compute_buffer' not in norm_compute_cache or len(
                norm_compute_cache['norm_tensors_compute_buffer']) != len(input_tensors):
            norm_compute_cache['norm_tensors_compute_buffer'] = [
                torch.empty([], dtype=torch.float, device=get_accelerator().current_device_name())
                for t in input_tensors
            ]
        compute_buffer = norm_compute_cache['norm_tensors_compute_buffer']
