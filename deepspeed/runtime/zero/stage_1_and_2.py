...
class DeepSpeedZeroOptimizer(ZeROOptimizer):
    ...
    def __init__(
        ...
        self.flatten = (
            ...
            if not self.zenflow
            else zenflow_utils._flatten_dense_tensors
        )
        self.unflatten = (
            _unflatten_dense_tensors
            if not self.zenflow
            else zenflow_utils._unflatten_dense_tensors
        )
        ...
        if self.reduce_scatter and self.uses_muon:
            assert (
                False
            ), f"{self.zero_stage_string} with reduce_scatter=True is incompatible with Muon optimizer. Please disable reduce_scatter or use a different optimizer."
        ...
        for i, param_group in enumerate(self.optimizer.param_groups):
            ...
            weights_partition = (
                ...
                .detach()
                .clone()
                .to(device=self.device, dtype=self.master_weights_and_grads_dtype)
            )

            if self.cpu_offload:
                if self.cpu_offload_pin_memory:
                    weights_partition = get_accelerator().pin_memory(weights_partition)
                temp_dtype = self.parallel_partitioned_bit16_groups[i][
                    partition_id
                ].dtype
                temp_buffer_bit16 = torch.full(
        ...
        if is_autocast_initialized():
            comm_dtypes = get_all_comm_dtypes(
                [p for params in self.bit16_groups for p in params]
            )
            if get_autocast_dtype() == torch.float16:
                self.torch_autocast_gradscaler = torch.amp.GradScaler(
        ...
        self.ipg_buckets: Dict[torch.dtype, IPGBucket] = {
            dtype: IPGBucket() for dtype in comm_dtypes
        }
            ...
            for i, _ in enumerate(self.bit16_groups):
                if i not in self.all_grad_tensors or self.all_grad_tensors[i] is None:
                    ...
                    for accumulated_grad, new_avg_grad in zip(
                        self.all_grad_tensors[i], avg_new
                    ):
                ...
                if param.requires_grad:
                    ...
                    def wrapper(param, i):
                        ...
                        def grad_handling_hook(*notneeded):
                            ...
                            self.update_hook_state_and_maybe_run_epilogue(
                                current_expected
                            )
    ...
    def reduce_independent_p_g_buckets_and_remove_grads(self, param, i):
        ...
        if self.contiguous_gradients:
            if param.numel() > self.reduce_bucket_size:
                ...
                new_grad_tensor = bucket.buffer[bucket.index].narrow(
                    0, bucket.elements, param.numel()
                )
                new_grad_tensor.copy_(
                    grad_reduc.view(-1)
                    if not self.zenflow
                    else grad_reduc.permute(*reversed(range(grad_reduc.ndim)))
                    .contiguous()
                    .view(-1)
                )
                grad_reduc.data = (
                    new_grad_tensor.data.view_as(grad_reduc)
                    if (not self.zenflow or grad_reduc.dim() == 1)
                    else new_grad_tensor.data.view_as(grad_reduc.transpose(0, 1))
                )
    ...
    def gradient_reduction_w_predivide(
        self, tensor, communication_data_type: torch.dtype
    ):
    ...
    def complete_grad_norm_calculation_for_cpu_offload(self, params):
        ...
        dist.all_reduce(
            total_dev_norm, op=dist.ReduceOp.SUM, group=self.dp_process_group
        )

        self._model_parallel_all_reduce(tensor=total_dev_norm, op=dist.ReduceOp.SUM)

        total_norm = total_dev_norm[0].item() ** (1.0 / norm_type)
        ...
        for comm_dtype in dtypes:
            ...
            if self.contiguous_gradients:
                if comm_dtype in self.extra_large_param_to_reduce:
                    ...
                    assert (
                        self.get_param_id(self.extra_large_param_to_reduce[comm_dtype])
                        == param_id
                    ...
                    extra_large_grad_reduc_for_average = (
                        extra_large_grad_reduc.view(-1)
                        if not self.zenflow
                        else extra_large_grad_reduc.permute(
                            *reversed(range(extra_large_grad_reduc.ndim))
                        )
                        .contiguous()
                        .view(-1)
                    )
                    extra_large_grad_reduc.data = (
                        extra_large_grad_reduc_for_average.data.view_as(
                            extra_large_grad_reduc
                        )
                        if (
                            not self.zenflow
                            or self.extra_large_param_to_reduce[comm_dtype].dim() == 1
                        )
                        else extra_large_grad_reduc_for_average.data.view_as(
                            extra_large_grad_reduc.transpose(0, 1)
                        )
                    )
                    ...
                    self.average_tensor(
                        bucket.buffer[bucket.index].narrow(0, 0, bucket.elements),
                        comm_dtype,
                    )
            else:
                self.buffered_reduce_fallback(
                    None, bucket.grads, comm_dtype, elements_per_buffer=bucket.elements
                )
    ...
    def reduce_ready_partitions_and_remove_grads(self, param, i):
        if (
            self.partition_gradients
            or self.is_gradient_accumulation_boundary
            or self.zenflow
        ):
    ...
    def get_flat_partition(
        ...
        if (
            (not self.optimizer.state[flatten_copy])
            and getattr(tensor_list[0], "use_muon", False)
            and "muon" in self.optimizer.__class__.__name__.lower()
        ):
            self.optimizer.state[flatten_copy] = {}
        if (
            "momentum_buffer" not in self.optimizer.state[flatten_copy]
            and getattr(tensor_list[0], "use_muon", False)
            and "muon" in self.optimizer.__class__.__name__.lower()
        ):
            ...
            self.optimizer.state[flatten_copy]["momentum_buffer"] = self.flatten(
                flatten_bf_list
            )
        ...
        for i, tensor in enumerate(tensor_list):
            ...
            if (
                getattr(tensor, "use_muon", False)
                and "muon" in self.optimizer.__class__.__name__.lower()
            ):
                assert (
                    tensor.ndim > 1
                ), f"if use muon, then tensor dim > 1, got {tensor.size()}"
                buffer = torch.narrow(
    ...
    def scaled_global_norm(self, norm_type=2):
        ...
        for i, group in enumerate(self.bit16_groups):
            if self.cpu_offload:
                norm = self.complete_grad_norm_calculation_for_cpu_offload(
                    self.params_in_partition[i]
                )
        ...
        for i, param_group in enumerate(self.bit16_groups):
            for param in param_group:
                if hasattr(param, "_hp_mapping") and param._hp_mapping is not None:
                    ...
                    if (
                        hasattr(param._hp_mapping, "optim_fragment")
                        and param._hp_mapping.optim_fragment is not None
                    ):
    ...
    def offload_states(
            ...
            if pin_memory:
                if not hasattr(self, "hp_params_pin_buffers"):
                    self.hp_params_pin_buffers = [
                        torch.empty_like(t, device=device).pin_memory()
                        for t in self.single_partition_of_fp32_groups
                    ]
                for src_tensor, dest_buf in zip(
                    self.single_partition_of_fp32_groups, self.hp_params_pin_buffers
                ):
        ...
        if needs_offload(OffloadStateTypeEnum.lp_params):
            ...
            if pin_memory:
                ...
                for src_tensor, dest_buf in zip(
                    self.bit16_groups_flat, self.lp_params_pin_buffers
                ):
                    dest_buf.copy_(src_tensor, non_blocking=non_blocking)
                    src_tensor.data = dest_buf
            else:
                ...
                if grad_list is not None:
                    for grad_tensor in grad_list:
                        if (
                            ...
                            and grad_tensor.device.type != device
                        ):
                            ...
                            grad_tensor.data = grad_tensor.data.to(
